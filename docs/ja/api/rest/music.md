...
ソースには 4 つの `GET` ルートだけがあります：`/proxy`、`/domains`、`/search`、`/play/netease/{song_id}`。これはファーストパーティー再生サービスで、無制限の取得プロキシではありません。`/proxy` は実行時の音楽ソース許可リストにある HTTP(S) 宛先だけを受け入れ、各リダイレクトを再検証し、非音声コンテンツを拒否して、下記の 50 MB 上限を適用します。統一認証のないメインサーバーポートを信頼できないクライアントへ公開しないでください。
