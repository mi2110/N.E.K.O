...
### 明示的な `recall_memory` ツール

`main_logic/core/tool_calling.py` は会話モデル向けに組み込みの `recall_memory` ツールを登録します。ハンドラーは `POST /query_memory/{lanlan_name}` を呼び出し、構造化された結果をモデルへ返します。モデルは自然言語の `query`、`time` 表現、またはその両方を指定できます。

| ツール引数 | 検索動作 |
|---|---|
| `query` のみ | アクティブな事実とリフレクションに対して BM25 と任意のコサイン検索を行い、Reciprocal Rank Fusion（RRF）で統合 |
| `time` のみ | 解析されたイベント時間ウィンドウに最も近い事実、リフレクション、アーカイブ済み事実を返す |
| `query` と `time` | イベント時間ウィンドウでハードフィルターした後、その範囲内でセマンティック検索 |

ペルソナは通常のダイアログコンテキストへすでにレンダリングされるため、意図的に検索プールから除外されています。アーカイブ済み事実は BM25 と時間検索には参加しますが、ベクトル検索には参加しません。ユーザー向けリコール経路では、ツールの待ち時間を抑えるため追加の LLM 再ランキングを行いません。

`memory/embeddings.py` の Embedding サービスは任意のローカルコンポーネントで、CPU ONNX execution provider を使用します。必要なランタイム、モデルバンドル、対応ハードウェア、または設定済みメモリ予算が利用できない場合は自動的に無効になり、ハイブリッド検索は BM25 のみで継続します。メモリサーバーへのリクエスト自体が失敗した場合、ツールは空結果を返し、会話を継続できるようにします。

組み込みツールの schema と LLM セッションの互換性を診断する場合に限り、`NEKO_DISABLE_BUILTIN_TOOLS=1` を使用してください。
...
- 通常の `INFO` リコールログには、モード、ヒット件数、処理時間などのメタデータだけが入り、生のクエリやリコール本文は入りません。診断用 `DEBUG` ログには、生のクエリと完全なツール引数が含まれる場合があります。
