...
24. 2026-06-19 追加复现：Day 7 仍出现教程模型不加载，且类似问题在多天教程里反复出现。它不是 Day 7 配置问题；Day 7 第一幕没有窗口/面板操作，所有每日 round 都共用 `TutorialRoundPreludeController.play()` 的 `beginAvatarOverride()` + `ensureTutorialYuiLive2dVisible()` 前奏。真正的通用层漏洞是 `isTutorialYuiLive2dActive()` 只看 `live2dManager._lastLoadedModelPath` / `modelRootPath` / `modelName`，而这些元数据可能在 reload、teardown 或半成功 load 后残留为 `yui-origin`；旧版 `ensureTutorialYuiLive2dVisible()` 因此会认为 YUI 已 active，只做 `ensureTutorialLive2dRenderActive()` 和 `applyTutorialLive2dViewportPlacement()`，即使 `getCurrentModel()` 已经为空、PIXI renderer/stage 不可用，或者布局返回 `false`，也会返回 true，导致流程继续但模型不可见。修复契约是：`isTutorialYuiLive2dActive()` 仍只表达路径/名称是否指向 YUI；不要把它改成视觉可渲染判断。新增 `getTutorialLive2dCurrentModel(manager)` 和 `hasTutorialYuiLive2dRenderableModel(manager)` 负责视觉对象验证，检查顺序为 manager -> currentModel -> `pixi_app.stage` -> `pixi_app.renderer`，任一缺失立即返回 false。`ensureTutorialYuiLive2dVisible()` 先算 `activeByPath = isTutorialYuiLive2dActive()`；只有 `activeByPath && hasTutorialYuiLive2dRenderableModel()` 时才执行 `ensureTutorialLive2dRenderActive()` 和 `applyTutorialLive2dViewportPlacement()`，且 placement 必须返回 true 才直接成功。`activeByPath` 为 true 但 renderable/placement 失败时必须重新 `loadTemporaryTutorialLive2dModel()`；直接 load 后仍没有 currentModel 必须抛 `tutorial_yui_live2d_model_missing_after_load`，不能让任何一天的新手教程带着空模型继续播放。
25. 2026-06-20 追加复现：重启 Day 2 新手教程后，模型旁边的 Live2D 悬浮按钮已经出现，但 YUI 模型本体没有显示。这个现象不能判定为模型文件未加载；按钮由 `Live2DManager.setupFloatingButtons(model)` 在 `_configureLoadedModel()` 中段创建，早于 canvas 最终揭示，且 `showLive2d()` 会单独把 `#live2d-floating-buttons` 设为 visible/opacity 1。明确代码证据是：旧版 `showLive2d()` 快速路径只按 `#live2d-container` 判断“模型已可见”，只恢复 `#live2d-canvas` 的 `visibility/pointer-events`，没有恢复 `opacity`；而 Live2D 加载预热、`body.yui-guide-live2d-preparing`、Day 2 最后一幕 `petalTransition: true` 对应的 `body.yui-guide-return-petal-fade`，都可能把 `#live2d-container/#live2d-canvas` 或 `--yui-guide-return-avatar-opacity` 留在透明态。修复契约是：`static/app-ui.js::showLive2d()` 快速路径必须调用 `restoreLive2DDisplaySurface('show-live2d-fast-path')`，清理 `yui-guide-live2d-preparing`、`yui-guide-return-petal-fade` 和 `--yui-guide-return-avatar-opacity`；container 只能移除残留 inline opacity 并回到基础 CSS，不能固定成 `opacity: 1 !important`，否则会压掉 `locked-hover-fade`；canvas 则可用 inline `!important` 强制恢复 opacity、visibility 和 pointer-events。`UniversalTutorialManager.clearTutorialLive2dPreparingStyles()` 必须同时清理 `yui-guide-return-petal-fade` 和 `--yui-guide-return-avatar-opacity`；`UniversalTutorialManager.ensureTutorialLive2dRenderActive(reason)` 在恢复 PIXI model/stage/ticker 前必须调用 `restoreTutorialLive2dDisplayState(reason)`，把 DOM canvas 也拉回可见。相关回归测试必须覆盖 `restoreLive2DDisplaySurface()`、`restoreTutorialLive2dDisplayState()` 和 `ensureTutorialLive2dRenderActive()` 的调用，防止以后只恢复按钮或 PIXI 而漏掉 canvas CSS。
