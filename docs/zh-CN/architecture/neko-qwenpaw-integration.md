...
持久化的 `core_config.json` 实际支持以下三个字段；`get_core_config()` 会把它们映射为适配器读取的 `OPENCLAW_*` 派生键：

| 配置 | 默认值 | 作用 |
| --- | --- | --- |
| `openclawUrl` | `http://127.0.0.1:8088` | 服务根地址；也可传入受支持的完整端点，适配器会还原根地址 |
| `openclawTimeout` | `300` 秒 | 单次任务超时 |
| `openclawDefaultSenderId` | `neko_user` | 无法从消息解析用户时的发送者 |

`QWENPAW_*`、`qwenpaw*`、鉴权 token 与 channel 名称虽然出现在适配器的内部兼容读取逻辑中，但当前不会由 `get_core_config()` 从持久化核心配置透传，因此不是可用的用户配置字段。
