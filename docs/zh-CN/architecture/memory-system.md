...
`main_logic/core/tool_calling.py` 会为对话模型注册内置 `recall_memory` 工具。处理器调用 `POST /query_memory/{lanlan_name}`，再把结构化结果返回给模型。模型可以提供自然语言 `query`、`time` 表达式，或同时提供两者：

| 工具参数 | 检索行为 |
|---|---|
| 仅 `query` | 在活跃事实与反思上并行执行 BM25 和可选余弦检索，再用倒数排名融合（RRF）合并 |
| 仅 `time` | 返回在事件时间上最接近所解析时间窗口的事实、反思和已归档事实 |
| `query` 与 `time` | 先按事件时间窗口硬过滤，再在窗口内执行语义检索 |

人格记忆被有意排除在召回池之外，因为它已经常规渲染进对话上下文。已归档事实参与 BM25 和时间召回，但不参与向量召回。面向用户的召回路径不会额外增加一轮 LLM 重排序请求，以控制工具延迟。

`memory/embeddings.py` 中的嵌入服务是可选的本地组件，使用 CPU ONNX execution provider。当所需运行时、模型包、受支持硬件或配置的内存预算不可用时，它会自行禁用，混合召回则继续以纯 BM25 模式工作。如果记忆服务器请求本身失败，工具会返回空结果提示，让对话继续进行。

只有在需要排查内置工具 schema 与 LLM 会话兼容性时，才应设置诊断开关 `NEKO_DISABLE_BUILTIN_TOOLS=1`。
...
- 常规 `INFO` 召回日志只包含模式、命中数量和耗时等元数据，不包含原始查询或召回文本。诊断级 `DEBUG` 日志可能包含原始查询和完整工具参数。
